...
- **F08:** DNS DKIM candidate selection now uses parsed tag semantics, accepts omitted `v=` while validating an explicit version, accepts RFC-valid tag whitespace such as `p = <key>`, and retains revoked/ambiguous-key rejection. Both versionless and whitespace-bearing DNS records are covered through verifier-level regressions.
